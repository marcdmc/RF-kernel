...
class RandomForestKernel:
    ...
    def RFKernelMatrix(self, rf, data):
        ...
        for grouping in groupings:
            ...
            """
            enum = enumerate(grouping) # Creates a list ((0,x0),(1,x1),...)
            print(enum)
            # For each pair of points compare their groupings and add to the kernel matrix
            for x, y in itertools.combinations(enum, 2):
                i = x[0]
                j = y[0]
                if x[1] == y[1]:
                    K[i][j] += 1
    ...
    def transform(self, X):
        ...
        return self.RFKernelMatrix(self.rf, X)